...
class CommandPaletteCommands(Provider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.hastin_app: HastinApp = self.app

    def async_command(self, key: str):
        """Helper function to call the process_key_event command asynchronously."""
        self.app.call_later(self.hastin_app.key_event_manager.process_key_event, key)

    def get_command_hits(self):
        """Helper function to get all commands and format them for discovery or search."""
        commands = self.hastin_app.command_manager.get_commands(
            self.hastin_app.tab_manager.active_tab.hastin.replay_file,
            self.hastin_app.tab_manager.active_tab.hastin.connection_source,
        )
