#!/usr/bin/env python3
"""Hastin - Your single pane of glass for real-time analytics into PostgreSQL.

Author: Hastin Contributors (forked from dolphie by Charles Thompson)
License: GPL-3.0
"""
...
from hastin.DataTypes import ConnectionSource, ConnectionStatus, HotkeyCommands
from hastin.Modules.ArgumentParser import ArgumentParser, Config
from hastin.Modules.CommandManager import CommandManager
from hastin.Modules.CommandPalette import CommandPaletteCommands
from hastin.Modules.KeyEventManager import KeyEventManager
...
from hastin.Panels import Dashboard as DashboardPanel
from hastin.Panels import Locks as LocksPanel
...
from hastin.Panels import Statements as StatementsPanel
from hastin.Widgets.CommandModal import CommandModal
from hastin.Widgets.TopBar import TopBar


def _get_version_from_pyproject() -> str:
    """Read version from pyproject.toml when package isn't installed."""
    try:
...
try:
    ...
    __package_name__ = "Hastin"
    __version__ = _get_version_from_pyproject()


class HastinApp(App):
    TITLE = "hastin"
    CSS_PATH = "Hastin.tcss"
    ...
    def __init__(self, config: Config):
        ...
        theme = RichTheme(
            {
                ...
                "purple": "#b565f3",
                "dark_gray": "#7a9a84",
                "b dark_gray": "b #7a9a84",
                "highlight": "#3dee82",
                "label": "#a8e4b8",
                "b label": "b #a8e4b8",
                "light_blue": "#98dca8",
                "b white": "b #e9e9e9",
                "b highlight": "b #3dee82",
                "b light_blue": "b #98dca8",
                "recording": "#ff5e5e",
        ...
        theme = TextualTheme(
            ...
            variables={
                ...
                "purple": "#b565f3",
                "dark_gray": "#7a9a84",
                "b_dark_gray": "b #7a9a84",
                "highlight": "#3dee82",
                "label": "#a8e4b8",
                "b_label": "b #a8e4b8",
                "light_blue": "#98dca8",
                "b_white": "b #e9e9e9",
                "b_highlight": "b #3dee82",
                "b_light_blue": "b #98dca8",
                "recording": "#ff5e5e",
        ...
        if config.daemon_mode:
            logger.info(
                f"Starting Hastin v{__version__} in daemon mode with a refresh interval of {config.refresh_interval}s"
            )
    ...
    def replay_pause(self, event: Button.Pressed):
        ...
        if not tab.hastin.pause_refresh:
            tab.hastin.pause_refresh = True
            self.notify("Replay is paused")
            event.button.label = "▶️  Resume"
        else:
            tab.hastin.pause_refresh = False
    ...
    def replay_pfs_metrics_radio_set_changed(self, event: RadioSet.Changed):
        ...
        if tab:
            self.refresh_panel(tab, tab.hastin.panels.pfs_metrics.name)
    ...
    def host_tab_changed(self, event: Tabs.TabActivated):
        ...
        if previous_tab and previous_tab.hastin.replay_file and previous_tab.worker:
        ...
        if (
            ...
            and tab.worker
            and (
                (tab.hastin.main_db_connection.is_connected() and tab.hastin.worker_processing_time)
                or tab.hastin.replay_file
            )
        ):
            ...
            for panel in tab.hastin.panels.get_all_panels():
                ...
                tab_panel.display = getattr(tab.hastin.panels, panel.name).visible

            tab.toggle_metric_graph_tabs_display()
            tab.toggle_entities_displays()

            if tab.hastin.connection_source in (
            ...
            elif tab.hastin.connection_source == ConnectionSource.pgbouncer:
                self.worker_data_processor.refresh_screen_pgbouncer(tab)
    ...
    def update_graphs(self, metric_tab_name: str):
        ...
        for metric_instance in tab.hastin.metric_manager.metrics.__dict__.values():
            if metric_tab_name == metric_instance.tab_name:
                ...
                with self.batch_update():
                    for graph_name in metric_instance.graphs:
                        getattr(tab, graph_name).render_graph(metric_instance, tab.hastin.metric_manager.datetimes)
                    self.update_stats_label(metric_tab_name)

    def update_stats_label(self, metric_tab_name: str):
        stat_data = {}

        for metric_instance in self.tab_manager.active_tab.hastin.metric_manager.metrics.__dict__.values():
            if metric_tab_name == metric_instance.tab_name:
    ...
    def toggle_panel(self, panel_name: str):
        ...
        getattr(self.tab_manager.active_tab.hastin.panels, panel_name).visible = new_display_status

        if panel_name not in [self.tab_manager.active_tab.hastin.panels.graphs.name]:
    ...
    def force_refresh_for_replay(self, need_current_data: bool = False):
        ...
        if tab.hastin.replay_file and (not tab.worker or not tab.worker.is_running):
            if tab.worker:
                tab.worker.cancel()
            if tab.worker_timer:
    ...
    def refresh_panel(self, tab: Tab, panel_name: str, toggled: bool = False):
        # Panel mapping for PostgreSQL connection sources
        pg_sources = (ConnectionSource.postgresql, ConnectionSource.rds, ConnectionSource.aurora)

        panel_mapping = {
            tab.hastin.panels.replication.name: dict.fromkeys(pg_sources, ReplicationPanel),
            tab.hastin.panels.dashboard.name: dict.fromkeys(pg_sources, DashboardPanel),
            tab.hastin.panels.processlist.name: dict.fromkeys(pg_sources, ProcesslistPanel),
            tab.hastin.panels.locks.name: dict.fromkeys(pg_sources, LocksPanel),
            tab.hastin.panels.statements.name: dict.fromkeys(pg_sources, StatementsPanel),
            tab.hastin.panels.pgbouncer.name: dict.fromkeys(pg_sources, PgBouncerPanel),
            # PgBouncer panels (standalone mode)
            tab.hastin.panels.pgbouncer_dashboard.name: {ConnectionSource.pgbouncer: PgBouncerDashboardPanel},
            tab.hastin.panels.pgbouncer_pools.name: {ConnectionSource.pgbouncer: PgBouncerPoolsPanel},
            tab.hastin.panels.pgbouncer_clients.name: {ConnectionSource.pgbouncer: PgBouncerClientsPanel},
            tab.hastin.panels.pgbouncer_servers.name: {ConnectionSource.pgbouncer: PgBouncerServersPanel},
        }

        for panel_map_name, panel_map_connection_sources in panel_mapping.items():
            panel_map_obj = panel_map_connection_sources.get(tab.hastin.connection_source)
            ...
            if panel_name == tab.hastin.panels.replication.name and toggled and tab.hastin.replication_status:
    ...
    def switch_changed(self, event: Switch.Changed):
        ...
        metric_instance = getattr(
            self.tab_manager.active_tab.hastin.metric_manager.metrics,
            ...
            if response.status_code == 200:
                ...
                if parse_version(latest_version) > parse_version(__version__):
                    self.notify(
                        ...
                        f"[$dark_gray]Find more details at https://github.com/euceph/hastin",
                        title="",
                        severity="information",
                        timeout=20,
                    )
                    ...
                    logger.info(
                        ...
                        "Find more details at https://github.com/euceph/hastin"
                    )
        except Exception:
            pass
    ...
    async def on_mount(self):
        ...
        if self.config.hostgroup:
            ...
            elif self.tab_manager.active_tab.hastin.replay_file:
                self.tab_manager.active_tab.replay_manager = ReplayManager(tab.hastin)
...
def main():
    ...
    app = HastinApp(arg_parser.config)
